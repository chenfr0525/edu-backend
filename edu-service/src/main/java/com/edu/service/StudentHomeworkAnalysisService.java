...
public class StudentHomeworkAnalysisService {
    ...
    public Map<String, Object> getStudentOverallAnalysis(Long studentId) {
        ...
        // 尝试从数据库获取缓存（7天内有效）
        AiAnalysisReport cachedReport = aiReportService.findLatestReport("STUDENT", studentId, "HOMEWORK");
        ...
        // 生成新的分析报告
        Map<String, Object> analysis = generateStudentOverallAnalysis(student);
...
analysis.setRank(rank);
    ...
    private AiSuggestionDTO getOrCreateAiSuggestion(Student student, Homework homework) {
        ...
        // 检查是否有缓存
        AiAnalysisReport cached = aiReportService.findLatestReport("STUDENT", 
                student.getId(), "HOMEWORK");
        
        if (cached != null && cached.getCreatedAt().isAfter(LocalDateTime.now().minusDays(3))) {
            suggestion.setSummary(cached.getSummary());
            suggestion.setActionItems(parseActionItems(cached.getSuggestions()));
            return suggestion;
        }
        ...
        if (!submission.isPresent() || submission.get().getScore() == null) {
            ...
            suggestion.setActionItems(Arrays.asList("请耐心等待老师批改"));
            suggestion.setNextStep("批改完成后可查看详细分析");
            return suggestion;
        }
        ...
        if (score >= 90) {
            ...
            suggestion.setActionItems(Arrays.asList("继续保持学习节奏", "尝试帮助其他同学", "挑战拓展题"));
            suggestion.setNextStep("建议预习下一章节内容");
        } else if (score >= 75) {
            ...
            suggestion.setActionItems(Arrays.asList("复习错题相关知识点", "多做同类题型练习", "参考优秀作业"));
            ...
            suggestion.setActionItems(Arrays.asList("重新复习课堂笔记", "观看教学视频", "向老师/同学请教"));
            suggestion.setNextStep("建议每天安排30分钟针对性练习");
        } else {
    ...
    private List<String> parseActionItems(String suggestions) {
    ...
    private Map<String, Object> generateStudentOverallAnalysis(Student student) {
        ...
        if (submissions.isEmpty()) {
            Map<String, Object> suggestionMap = new HashMap<>();
             suggestionMap.put(  "summary", "暂无作业数据，无法生成分析报告");
             suggestionMap.put(  "suggestions", "请先完成并提交作业");
